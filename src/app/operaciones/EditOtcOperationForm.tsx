...
import { updateOtcOperation } from "./actions";
...
type Props = {
  ...
  initialUsdtBackend: string;
  initialRateBackend: string;
...
export function EditOtcOperationForm({
  operationId,
  clients,
  operators,
  bankAccounts,
  ...
  const [side, setSide] = useState<OtcSide>(initialSide);
  const isBuy = side === OtcSide.CLIENT_BUYS_USDT;

  useEffect(() => {
    if (!isBuy) {
      setLineIds([0]);
      setLineMeta({ 0: defaultLineMeta() });
      setAllocBackends({});
    }
  }, [isBuy]);

  const nextLineId = useRef(
    initialAllocations.length > 0 ? initialAllocations.length : 1,
  );
  const [lineIds, setLineIds] = useState<number[]>(() =>
    initialAllocations.length > 0 ? initialAllocations.map((_, i) => i) : [0],
  );
  const [lineMeta, setLineMeta] = useState<Record<number, OtcAllocLineMeta>>(() => {
    const m: Record<number, OtcAllocLineMeta> = {};
    if (initialAllocations.length > 0) {
      initialAllocations.forEach((a, i) => {
        m[i] = {
          dest: a.destination,
          operatorPayout:
            a.destination === "OPERATOR" && a.currency === "USDT" ? "USDT" : "GTQ",
        };
      });
    } else {
      m[0] = defaultLineMeta();
  ...
  const [totalFiatBackend, setTotalFiatBackend] = useState(initialTotalFiatBackend);
  const [rateFiatBackend, setRateFiatBackend] = useState(initialRateBackend);
  const [usdtDeliveredBackend, setUsdtDeliveredBackend] = useState(initialUsdtBackend);
  const [allocBackends, setAllocBackends] = useState<Record<number, string>>(() => {
    const b: Record<number, string> = {};
    initialAllocations.forEach((a, i) => {
      b[i] = a.amount.toString();
    });
    return b;
  });

  const allocLineDefaults: Record<number, OtcAllocLineDefaults> = useMemo(() => {
    const d: Record<number, OtcAllocLineDefaults> = {};
    initialAllocations.forEach((a, i) => {
      d[i] = {
        operatorId: a.operatorId ?? undefined,
        bankAccountId: a.bankAccountId ?? undefined,
        amountBackend: a.amount.toString(),
        ref: a.reference ?? undefined,
  ...
  const allocationHint = useMemo(() => {
    ...
    if (!totalFiatBackend.trim() && sumEquiv.isZero()) {
      return {
        tone: "muted" as const,
        text: "Indique el total GTQ y reparto.",
      };
    }
    if (
      lineIds.some((id) => {
      ...
      return { tone: "warn" as const, text: "Indique la tasa GTQ/USDT para líneas operador en USDT." };
    }
    if (diff.abs().lte(tol)) {
      return { tone: "ok" as const, text: "Reparto cuadrado con el total GTQ." };
    }
    if (diff.lt(0)) {
      return {
        tone: "warn" as const,
        text: `Faltan Q${formatMoneyAmountCoreDisplay(Number(diff.abs().toString()))} en equivalente GTQ.`,
      };
    }
    return {
      tone: "warn" as const,
      text: `Sobran Q${formatMoneyAmountCoreDisplay(Number(diff.toString()))} en equivalente GTQ.`,
    };
    ...
    <form action={formAction} className="mt-4 space-y-4 rounded border border-zinc-200 bg-white p-4 text-sm">
      ...
      <label className="block">
        ...
        <select
          name="side"
          value={side}
          onChange={(e) => setSide(e.target.value as OtcSide)}
          className="mt-1 w-full rounded border border-zinc-400 px-2 py-1"
        >
          <option value={OtcSide.CLIENT_BUYS_USDT}>Cliente compra USDT</option>
          <option value={OtcSide.CLIENT_SELLS_USDT}>Cliente vende USDT</option>
        </select>
      </label>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="block">
          {isBuy ? "USDT entregado" : "USDT recibido"}
          <div className="mt-1">
            <MoneyInput
              name="usdtAmount"
              currency="USDT"
              required
              defaultValue={initialUsdtBackend}
        ...
        <label className="block">
          ...
          <div className="mt-1">
            <MoneyInput
              ...
              mode="rate"
              required
              defaultValue={initialRateBackend}
      ...
      <label className="block">
        {isBuy ? "Total recibido GTQ" : "Total GTQ pagado"}
        <div className="mt-1">
          <MoneyInput
            name={isBuy ? "gtqRecibidoTotal" : "totalFiat"}
            currency="GTQ"
            required
            defaultValue={initialTotalFiatBackend}
      ...
      {isBuy && partialDeliveryPreview && partialDeliveryPreview.pending > 0.01 ? (
        <div className="rounded border border-amber-300 bg-amber-50 p-3 text-xs text-amber-950">
          ...
          <p className="mt-1 tabular-nums">
            GTQ aplicado hoy (USDT × tasa): Q{formatMoneyAmountCoreDisplay(partialDeliveryPreview.applied)} · Pendiente:
            Q{formatMoneyAmountCoreDisplay(partialDeliveryPreview.pending)}
      ...
      {isBuy ? (
        <fieldset className="space-y-2 rounded border border-amber-200 bg-amber-50/40 p-3">
          <legend className="px-1 text-xs font-semibold text-amber-950">Reparto</legend>
          <OtcAllocationLineFields
            lineIds={lineIds}
            lineMeta={lineMeta}
            ...
            allocLineDefaults={allocLineDefaults}
          />
          {allocationHint ? (
            <p
      ...
      <label className="block">
        Notas (opcional)
        <textarea name="notes" rows={2} defaultValue={initialNotes} className="mt-1 w-full rounded border border-zinc-400 px-2 py-1" />
      </label>
